...
export const dashboardRouter = createTRPCRouter({
  ...
  stats: protectedProcedure.query(async () => {
    const [totalRows, activeRows, thisMonthRows, lastMonthRows] = await Promise.all([
      db.select({ count: count() }).from(dosya),
      db.select({ count: count() }).from(dosya).where(eq(dosya.durum, 'aktif')),
      db.select({ count: count() }).from(dosya).where(
        sql`strftime('%Y-%m', ${dosya.created_at}) = strftime('%Y-%m', 'now')`
      ...
      db.select({ count: count() }).from(dosya).where(
        sql`strftime('%Y-%m', ${dosya.created_at}) = strftime('%Y-%m', 'now', '-1 month')`
      ),
    ])

    const totalDosya = totalRows[0]?.count ?? 0
    const aktivDosya = activeRows[0]?.count ?? 0
    const buAyAcilan = thisMonthRows[0]?.count ?? 0
    const gecenAyAcilan = lastMonthRows[0]?.count ?? 0
