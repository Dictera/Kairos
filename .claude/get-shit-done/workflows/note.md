...
- **Global scope**: `D:/sigorta-takip/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/sigorta-takip/.claude/notes/`)
...
2. Glob `D:/sigorta-takip/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/sigorta-takip/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/sigorta-takip/.claude/notes/` — works in any directory
