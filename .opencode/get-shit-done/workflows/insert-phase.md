...
```bash
INIT=$(node "D:/sigorta-takip/.opencode/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "D:/sigorta-takip/.opencode/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
